...
interface RecipeIngredient {
  id: number // ID of the recipe_ingredient entry
  recipe_id: number
  ingredient_id: number
  ingredient_name: string // From ingredients table
  quantity_used: number
  unit_used: string
  cost_per_unit: number // From inventory_stock_levels or ingredients
  total_cost: number
  ingredient_base_unit: string // From ingredients table
}
...
export async function getRecipeIngredients(recipeId: number): Promise<RecipeIngredient[]> {
  try {
    ...
    // Verify the recipe belongs to the restaurant
    const recipeCheck = await sql`
      SELECT id FROM recipes WHERE id = ${recipeId} AND restaurant_id = ${restaurantId}
    `
    if (recipeCheck.length === 0) {
      throw new Error("Recipe not found or does not belong to this restaurant.")
    }
    ...
    const result = await sql`
      SELECT
        ...
        i.unit_of_measure as ingredient_base_unit,
        COALESCE(isl.cost_per_storage_unit, 0) as cost_per_unit,
        (ri.quantity_used * COALESCE(isl.cost_per_storage_unit, 0)) as total_cost
...
export async function updateRecipeIngredients(
  recipeId: number,
  ingredients: Array<{ ingredient_id: number; quantity_used: number; unit_used: string }>,
) {
  try {
    const restaurantId = await getRestaurantIdFromSession()
    if (!restaurantId) {
      throw new Error("Authentication required to update recipe ingredients.")
    }
    ...
    await sql.begin(async (sql) => {
      ...
      const currentIngredients = await sql`
        SELECT ingredient_id FROM recipe_ingredients WHERE recipe_id = ${recipeId}
      `
      const currentIngredientIds = new Set(currentIngredients.map((ing) => ing.ingredient_id))

      const newIngredientIds = new Set(ingredients.map((ing) => ing.ingredient_id))

      // 1. Delete ingredients that are no longer in the list
      const ingredientsToDelete = currentIngredients.filter((ing) => !newIngredientIds.has(ing.ingredient_id))
      if (ingredientsToDelete.length > 0) {
        const idsToDelete = ingredientsToDelete.map((ing) => ing.ingredient_id)
        await sql`
          DELETE FROM recipe_ingredients
          WHERE recipe_id = ${recipeId} AND ingredient_id IN (${sql(idsToDelete)})
        `
      }

      // 2. Add or update ingredients
      for (const ingredient of ingredients) {
        // Verify the ingredient belongs to this restaurant
        const ingredientCheck = await sql`
          SELECT id FROM ingredients WHERE id = ${ingredient.ingredient_id} AND restaurant_id = ${restaurantId}
        `
        if (ingredientCheck.length === 0) {
          throw new Error(
            `Ingredient with ID ${ingredient.ingredient_id} not found or does not belong to this restaurant.`,
          )
        }

        if (currentIngredientIds.has(ingredient.ingredient_id)) {
          ...
          await sql`
            ...
            SET
              ...
              updated_at = CURRENT_TIMESTAMP
            WHERE recipe_id = ${recipeId} AND ingredient_id = ${ingredient.ingredient_id}
          `
        } else {
          // Add new ingredient
          await sql`
            INSERT INTO recipe_ingredients (recipe_id, ingredient_id, quantity_used, unit_used)
            VALUES (${recipeId}, ${ingredient.ingredient_id}, ${ingredient.quantity_used}, ${ingredient.unit_used})
          `
        }
      }
    })
    ...
    revalidatePath("/dashboard/operations-hub/recipes")
    revalidatePath(`/dashboard/operations-hub/recipes/${recipeId}`) // Revalidate specific recipe page
    return { success: true }
  } catch (error: any) {
    console.error(`Error updating ingredients for recipe ${recipeId}:`, error)
    throw error
  }
}
