...
interface CategoryFormDialogProps {
  ...
  children?: React.ReactNode // Add children prop for trigger element
}
...
export function CategoryFormDialog({
  ...
  const handleCreateGlobalCategory = async (e: React.FormEvent) => {
    ...
    try {
      await createCategory({
        name: newGlobalCategoryName.trim(),
        type: "menu_item",
        order_index: 0,
      })
  ...
  const handleAddCategoryToMenu = async (categoryId: number) => {
    if (!digitalMenuId) {
      toast({
        title: "Error",
        description: "No se ha seleccionado un menú digital.",
        variant: "destructive",
      })
      return
    }
  ...
  // Safely filter categories not in menu
  ...
  return (
    <>
      {children && <div onClick={() => setIsDialogOpen(true)}>{children}</div>}
